#!/usr/bin/env bash
# douyin-transcript :: resolve + download + extract audio + local Whisper transcribe.
# Prints WORKDIR + metadata + RAW transcript to stdout. Leaves files in a /tmp mktemp
# WORKDIR for the agent to read; the agent removes WORKDIR after writing the output .md.
#
# Usage:  dl-transcribe.sh "<douyin url or full 复制打开抖音… share text>"
# Output markers:  WORKDIR= / ----META---- / ----RAW---- / ----END----
...
RAW_INPUT="${1:?usage: dl-transcribe.sh <douyin-url-or-share-text>}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MODEL="${WHISPER_MODEL_PATH:-$HOME/.cache/whisperkit-models/openai_whisper-large-v3-v20240930}"
LANG="${WHISPER_LANG:-zh}"
# Browsers to try for cookies if the first download is risk-controlled. Override with a
# single browser, e.g. DOUYIN_COOKIE_BROWSER=safari.
COOKIE_BROWSERS="${DOUYIN_COOKIE_BROWSER:-chrome safari firefox edge arc brave}"
...
# --- resolve URL out of pasted share text (short link or full video URL) ---
URL=$(printf '%s' "$RAW_INPUT" | grep -oE 'https?://[a-zA-Z0-9./_?=&%~-]*douyin\.com[a-zA-Z0-9./_?=&%~-]*' | head -1 || true)
[ -z "$URL" ] && URL=$(printf '%s' "$RAW_INPUT" | grep -oE 'https?://[a-zA-Z0-9./_?=&%~-]+' | head -1 || true)
[ -z "$URL" ] && { echo "NO_URL"; exit 2; }

WORKDIR=$(mktemp -d /tmp/douyin-transcript.XXXXXX)
...
# --- download metadata + 16kHz mono WAV; retry over browsers on a_bogus/risk-control ---
fetch() {
...
[ -f "$WORKDIR/audio.wav" ] || { echo "DOWNLOAD_FAILED url=$URL (Douyin risk-control; try a recent yt-dlp, set DOUYIN_COOKIE_BROWSER=<your-logged-in-browser>, or a parse-API fallback)"; exit 5; }
